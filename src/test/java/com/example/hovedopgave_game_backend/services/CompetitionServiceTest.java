...
//DataJpaTest makes it possible to us our own database
@DataJpaTest
//makes a database in memory with configuration that we use in production - from changelogs and mockdata
...
class CompetitionServiceTest {
    ...
    void createCompetitionWithDefaultQuizzes() {
        ...
        //checks if competition not null
        assertNotNull(savedCompetition);

        //checks if there is 2 quizzes in list
        assertEquals(2,quizList.size());

        //checks all answers are added to the list
        assertEquals(8, answers.size());

        //checks if methods return null, if organizer does not exist i db
        assertNull(badRequestCompetition);

        //checks if it is the acurate quizzes created to the competition
